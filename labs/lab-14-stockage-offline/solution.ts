// =============================================================================
// Lab 14 — Stockage local et offline-first (Solution)
// =============================================================================
// Execution : npx tsx labs/lab-14-stockage-offline/solution.ts
// =============================================================================
...
const { test, run } = createTestRunner('Lab 14 — Stockage local et offline-first (Solution)');
...
// Queue FIFO avec un tableau interne. On expose aussi une methode interne
// _pushRaw pour permettre au SyncManager de re-inserer des items avec
// leur retryCount preserve.
// =============================================================================

function createOfflineQueue<T = unknown>(): OfflineQueue<T> {
  const items: QueueItem<T>[] = [];
  let idCtr = 0;

  const queue: OfflineQueue<T> = {
    enqueue(action: string, payload: T): string {
      const id = `q_${++idCtr}_${Date.now()}`;
      items.push({ id, action, payload, timestamp: Date.now(), retryCount: 0 });
      return id;
    },
  ...
  // Methode interne pour re-inserer un item complet (avec retryCount)
  (queue as any)._pushRaw = (item: QueueItem<T>) => {
    items.push({ ...item, id: `q_${++idCtr}_${Date.now()}` });
  };
...
function createSyncManager<T = unknown>(
  queue: OfflineQueue<T>,
  apiFn: (item: QueueItem<T>) => Promise<boolean>,
): SyncManager<T> {
  let status: SyncStatus = 'idle';
  const pushRaw = (queue as any)._pushRaw as ((item: QueueItem<T>) => void) | undefined;
  ...
  return {
    async sync(): Promise<SyncResult> {
      ...
      for (const item of items) {
        const success = await apiFn(item);
        if (success) {
          synced++;
        } else {
          failed++;
          errors.push(`Failed to sync item ${item.id}`);
          // Re-inserer avec retryCount incremente
          if (pushRaw) {
            pushRaw({ ...item, retryCount: item.retryCount + 1 });
          } else {
            queue.enqueue(item.action, item.payload);
          }
        }
      }
    ...
    async retry(id: string): Promise<boolean> {
      const items = queue.flush();
      let target: QueueItem<T> | null = null;

      for (const item of items) {
        if (item.id === id && !target) {
          target = item;
        } else if (pushRaw) {
          pushRaw(item);
        } else {
...
function conflictResolver(
  ...
  switch (strategy) {
    case 'client-wins':
      return { ...local, version: nextVersion };
    case 'server-wins':
      return { ...remote, version: nextVersion };
    case 'merge':
      return {
        id: local.id,
...
function createMigrationRunner(
  ...
  return {
    up(): Record<string, unknown>[] {
      const next = sorted.find((m) => m.version > currentVersion);
      if (!next) return data;
      data = next.up(data);
    ...
    down(): Record<string, unknown>[] {
      if (currentVersion === 0) return data;
      const cur = sorted.find((m) => m.version === currentVersion);
      if (!cur) return data;
      data = cur.down(data);
      const prev = sorted.filter((m) => m.version < cur.version);
      currentVersion = prev.length > 0 ? prev[prev.length - 1].version : 0;
      return data;
    },
...
// Map preserves insertion order. L'item le plus ancien est en premiere position,
// le plus recent en derniere. get() deplace l'item en fin de Map.
// =============================================================================

function createLRUCache<T>(maxSize: number): LRUCache<T> {
  ...
  return {
    get(key: string): T | null {
      ...
      cache.delete(key);
      cache.set(key, value); // Repositionner en fin (MRU)
      return value;
    },

    set(key: string, value: T): void {
      cache.delete(key); // Supprimer si existant
      if (cache.size >= maxSize) {
        // Evincer le LRU (premier dans la Map)
    ...
    keys(): string[] {
      // Map itere du plus ancien au plus recent ; on inverse
      return [...cache.keys()].reverse();
    },
  };
}
...
test('Ex3: server-wins retourne la version distante', () => {
  ...
  assertEqual(resolved.version, 6);
});
...
test('Ex3: merge fusionne les deux versions', () => {
  ...
  assertEqual(resolved.updatedAt, 1200);
  assertEqual(resolved.version, 5);
  assertEqual(resolved.id, '1');
});

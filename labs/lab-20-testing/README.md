...
Simulez un store Redux/Zustand avec tracking des actions dispatchees. Les reducers optionnels sont appliques au dispatch. Implementez `subscribe` avec desinscription et `reset` pour revenir a l'etat initial.

### Exercice 3 : createMockApi (4 tests)

Simulez une API HTTP avec des handlers predetermines (GET, POST, DELETE). Retournez 404 pour les routes non configurees. Enregistrez chaque requete dans un log consultable avec `getRequestLog`.

### Exercice 4 : createSnapshotManager (4 tests)

Gerez des snapshots comme Jest : `take` (creer), `compare` (verifier la correspondance), `update` (creer ou modifier) et `delete`. `take` leve une erreur si le snapshot existe deja.

  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        if (_baseDelay > 0) {
          await new Promise(r => setTimeout(r, _baseDelay * Math.pow(2, attempt)));
        }
      }
    }
  }
...
runner.test('createApiClient: GET avec params ajoute les query strings', async () => {
  const client = createApiClient('https://api.example.com');
  const result = await client.get<RequestConfig>('/users', {
...
runner.test('createApiClient: POST serialise le body en JSON', async () => {
  const client = createApiClient('https://api.example.com');
  const result = await client.post<RequestConfig>('/users', { name: 'Alice', email: 'a@b.com' });
  assertEqual(result.data.method, 'POST');
  assertEqual(result.data.body, JSON.stringify({ name: 'Alice', email: 'a@b.com' }));
...
runner.test('createApiClient: intercepteurs appliques dans l\'ordre', async () => {
  const client = createApiClient('https://api.example.com');
  client.addRequestInterceptor((config) => ({
    ...config,
...
runner.test('createTokenManager: getToken retourne le token initial', () => {
  const tm = createTokenManager('initial-token', Date.now() + 60000, async () => ({
    token: 'new', expiresAt: Date.now() + 60000,
  }));
  assertEqual(tm.getToken(), 'initial-token');
});

runner.test('createTokenManager: isExpired retourne false si non expire', () => {
...
runner.test('createTokenManager: refresh met a jour le token', async () => {
  const tm = createTokenManager('old', Date.now() - 1000, async () => ({
    token: 'refreshed', expiresAt: Date.now() + 60000,
  }));
...
runner.test('createTokenManager: refresh deduplication', async () => {
  ...
  const [r1, r2, r3] = await Promise.all([tm.refresh(), tm.refresh(), tm.refresh()]);
...
runner.test('createAbortableRequest: resolve avant timeout', async () => {
  const { promise } = createAbortableRequest(
    async () => 'done',
    5000
  );

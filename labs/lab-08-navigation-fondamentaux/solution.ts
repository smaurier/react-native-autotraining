// =============================================================================
// Lab 08 — React Navigation fondamentaux (Solution)
// =============================================================================
// Execution : npx tsx labs/lab-08-navigation-fondamentaux/solution.ts
// =============================================================================
...
const { test, run } = createTestRunner('Lab 08 — React Navigation fondamentaux (Solution)');
...
function createNavigationStack(initialRoute: Route) {
  const routes: Route[] = [{ ...initialRoute }];
  ...
  return {
    ...
    pop() {
      if (routes.length <= 1) {
        throw new Error('Cannot pop the last route in the stack');
      }
      routes.pop();
    },

    replace(name: string, params?: Record<string, unknown>) {
      routes[routes.length - 1] = { name, params };
    },

    reset(newRoutes: Route[]) {
      if (!newRoutes || newRoutes.length === 0) {
        throw new Error('Cannot reset with an empty routes array');
      }
      routes.length = 0;
      newRoutes.forEach((r) => routes.push({ ...r }));
    },
...
function createTypedRouter(
  routeDefinitions: Record<string, string[] | null>
) {
  const visitedRoutes: Array<{ name: string; params?: Record<string, unknown> }> = [];

  return {
    navigate(name: string, params?: Record<string, unknown>) {
      if (!(name in routeDefinitions)) {
        throw new Error(`Route "${name}" is not defined`);
      }

      const requiredParams = routeDefinitions[name];
      if (requiredParams !== null) {
        if (!params) {
          throw new Error(
            `Route "${name}" requires params: ${requiredParams.join(', ')}`
          );
        }
        for (const param of requiredParams) {
          if (!(param in params)) {
            throw new Error(
              `Missing required param "${param}" for route "${name}"`
            );
          }
        }
      }

      visitedRoutes.push({ name, params });
    },
    ...
    getCurrentRoute() {
      if (visitedRoutes.length === 0) return undefined;
      return visitedRoutes[visitedRoutes.length - 1];
    },
  };
}
...
function serializeNavigationState(state: NavigationState): string {
  return JSON.stringify(state);
}

function deserializeNavigationState(json: string): NavigationState {
  let parsed: unknown;
  try {
  ...
  if (!Array.isArray(obj.routes)) {
    throw new Error('Navigation state must have a routes array');
  }

  if (obj.routes.length === 0) {
    throw new Error('Navigation state routes cannot be empty');
  }

  if (typeof obj.index !== 'number' || obj.index < 0 || obj.index >= obj.routes.length) {
    throw new Error('Navigation state index is out of bounds');
  }
...
function createScreenOptions(
  ...
  overrides: ScreenOptions
): ScreenOptions {
  const result: ScreenOptions = { ...defaults };
...
test('Ex2: navigate vers une route inexistante leve une erreur', () => {
  ...
  assertThrows(() => router.navigate('Unknown'));
});
...
test('Ex2: navigate avec parametres manquants leve une erreur', () => {
  ...
  assertThrows(() => router.navigate('Details', { id: 42 }));
});
